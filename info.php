...
$name = $id= $department = $batch = $mobile = $email = $gender = $website =$hometown=$currentcity=$company=$designation=$jobcountry=$bscuniversity=$bscsubject=$bscyear=$mscuniversity=$mscsubject=$mscyear=$phduniversity=$phdsubject=$phdyear=$researcharea=$researchgate="";
...
*/
  ...
  if($status == "OK")
  {
	...
	$q = "update users set name= '$name' where username='$username' ";
	mysqli_query($db,$q);
...
<form style="margin:auto;width:50%;border:5px solid MediumSeaGreen;" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
	...
	<div style="text-align:center;background-color:#ccc;font-size:30px;">Your Information</div>
  ...
  Present address: <input type="text" name="currentcity" value="<?php echo $currentcity;?>">
  <br><br>
